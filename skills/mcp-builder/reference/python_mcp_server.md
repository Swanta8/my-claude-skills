...
> **Start here:** copy `../templates/python/` and adapt. This guide is the reference for *why* the template looks the way it does and how to extend it.
...
### Testing
- [ ] `python -m py_compile server.py` exits clean (syntax + import check)
- [ ] `npx @modelcontextprotocol/inspector python server.py` starts and lists all tools
- [ ] Each tool invoked once in Inspector with realistic input returns the expected shape
- [ ] Error scenarios (bad input, 401/404/429) return actionable messages
- [ ] Do NOT test by running `python server.py` directly — stdio servers hang waiting for stdin